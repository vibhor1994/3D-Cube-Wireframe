...
decelRate :: Number
decelRate = 30.0 / toNumber framesPerSecond


plotCube :: forall e. Eff (dom :: DOM |e) Unit
plotCube = do
...
initialSpeed :: forall eff h. STRef h RotationVector
  -> STRef h { x::Number, y::Number } -> STRef h Boolean
  -> Eff (dom :: DOM, st :: ST h, timer :: TIMER, console :: CONSOLE  | eff) Unit
initialSpeed velocityRef mousePosRef runFlagRef = do
...
startMouseHandlers transformRef velocityRef = do
  ...
  let downHandler event jq = do
        ...
        decelerator
        initialSpeed velocityRef mousePosRef runFlagRef
